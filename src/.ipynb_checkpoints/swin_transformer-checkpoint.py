...
from .resnext50_unet import Resnext50Unet
from .resnet152_unet import Resnet152Unet
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        x = (attn @ v).transpose(1, 2).reshape(B_, N, C)  # B, nH, Wh*Ww, C//nH
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
...
class SwinTransformer1(nn.Module):
    ...
    def __init__(self, img_size=512, patch_size=1, in_chans=192, num_classes=2,
    ...
    def forward(self, x):
        ...
        # model1.load_state_dict(torch.load(weights_path1)['model'], strict=False)
        # model1 = nn.DataParallel(model1, device_ids=[0,1,6,7])
        # model1.to(device)
        # model2.load_state_dict(torch.load(weights_path2)['model'], strict=False)
        # model2 = nn.DataParallel(model2, device_ids=[0,1,6,7])
        # model2.to(device)
        # model3.load_state_dict(torch.load(weights_path3)['model'], strict=False)  # , map_location='cpu'
